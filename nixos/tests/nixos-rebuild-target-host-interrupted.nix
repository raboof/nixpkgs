  ...
  testScript =
        ...
        hostPkgs.writeText "configuration.nix" # nix
          ''
            { lib, pkgs, modulesPath, ... }: {
              ...
              # may be needed to make NIX_SSHOPTS work for nix-copy-closure in nixos-rebuild,
              # but seems to cause rebuilds during the test...
              nix.package = pkgs.nixVersions.latest;

              # change to make sure it is restarted during the 'switch':
    ...
    ''
      ...
      with subtest("Deploy to alice@target via reverse ssh"):
        ...
        # haven't found a nicer way to wait until the switch is complete...
        deployer.sleep(20)
        target.wait_for_unit("autossh-ng.service")
